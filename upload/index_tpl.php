				...
				<?php foreach ($criteria as $v): ?>
					<div>
						<div class="items dflex">
							<a class="img" href="<?=$configBase?><?= $v['link'] ?>">
...
<?php if (!empty($pronb)) { ?>
	<div class="wrap-pronb">
		<div class="wrap-content">
			...
			<div class="slide-4 none">
				<?php foreach ($pronb as $v) { ?>
					<div>
						<div class="item-pronb">
							<a class="scale-img" href="<?= $v[$sluglang] ?>" title="<?= $v['name'.$lang] ?>">
								<?= $func->getImage(['class' => 'w-100','sizes' => '310x310x1', 'isWatermark' => true, 'prefix' => 'product', 'upload' => UPLOAD_PRODUCT_L, 'image' => $v['photo'], 'alt' => $v['name' . $lang]]) ?>
							</a>
							<div class="content-pronb">
								<h3 class="name-pronb"><a class="text-split text-decoration-none" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>"><?= $v['name' . $lang] ?></a></h3>
								<p class="price-pronb">
