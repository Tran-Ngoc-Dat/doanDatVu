<?php
	...
	function saveAdmin()
	{
		...
		if($id)
		{
			if($func->checkRole())
			{
				$row = $d->rawQueryOne("select id, role from user where role = 1 and id = ? limit 0,1",array($id));
	...
	function saveMember()
	{
		...
		if($id)
		{
			if($func->checkRole())
			{
				$row = $d->rawQueryOne("select id,role from user where role = 0 and id = ? limit 0,1",array($id));
