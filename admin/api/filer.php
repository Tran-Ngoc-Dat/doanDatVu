<?php
include "config.php";

$id = (!empty($_POST['id'])) ? htmlspecialchars($_POST['id']) : 0;
$id_product = (!empty($_POST['id_product'])) ? htmlspecialchars($_POST['id_product']) : 0;
$folder = (!empty($_POST['folder'])) ? htmlspecialchars($_POST['folder']) : '';
$info = (!empty($_POST['info'])) ? htmlspecialchars($_POST['info']) : '';
$value = (!empty($_POST['value'])) ? htmlspecialchars($_POST['value']) : '';
$listid = (!empty($_POST['listid'])) ? $func->sanitize($_POST['listid']) : '';
$com = (!empty($_POST['com'])) ? htmlspecialchars($_POST['com']) : '';
$kind = (!empty($_POST['kind'])) ? htmlspecialchars($_POST['kind']) : '';
$type = (!empty($_POST['type'])) ? htmlspecialchars($_POST['type']) : '';
$colfiler = (!empty($_POST['colfiler'])) ? htmlspecialchars($_POST['colfiler']) : '';
$cmd = (!empty($_POST['cmd'])) ? htmlspecialchars($_POST['cmd']) : '';
$hash = (!empty($_POST['hash'])) ? htmlspecialchars($_POST['hash']) : '';
$gallery = null;

if ($cmd == 'refresh' && ($id_product > 0)) {
    if ($id_product) {
        $gallery = $d->rawQuery("select * from gallery where id_product = ? order by id desc", array($id_product));
    } 

    if ($gallery) {
        for ($i = 0; $i < count($gallery); $i++) {
            echo $func->galleryFiler($gallery[$i]['numb'], $gallery[$i]['id'], $gallery[$i]['photo'], $gallery[$i]['name'], $com, $colfiler);
        }

        $cache->delete();
    }
} else if ($cmd == 'info' && $id > 0 && ($id_product > 0 )) {
    if ($info == 'numb') {
        $data['numb'] = $value;
    }

    if ($info == 'name') {
        $data['name'] = $value;
    }

    $d->where('id', $id);
    if ($d->update('gallery', $data)) {
        if ($id_product) {
            $gallery = $d->rawQuery("select * from gallery where id_product = ? order by id desc", array($id_product));
        }
        if ($gallery) {
            for ($i = 0; $i < count($gallery); $i++) {
                echo $func->galleryFiler($gallery[$i]['numb'], $gallery[$i]['id'], $gallery[$i]['photo'], $gallery[$i]['name'], $com, $colfiler);
            }

            $cache->delete();
        }
    }
} else if ($cmd == 'updateNumb' && $id_product > 0) {
    if ($id_product) {
        $row = $d->rawQuery("select id from gallery where id_product = ? order by id desc", array($id_product));
    } 
} else if ($cmd == 'delete' && $id > 0) {
    $row = $d->rawQueryOne("select photo from gallery where id = ? limit 0,1", array($id));

    $path = "../../upload/" . $folder . "/" . $row['photo'];

    $func->deleteFile($path);

    $d->rawQuery("delete from gallery where id = ?", array($id));

    $cache->delete();
} else if ($cmd == 'delete-all' && $listid != '') {
    $listid = explode(",", $listid);
    $cols = ["id", "photo"];
    $d->where('id', $listid, 'IN');
    $row = $d->get("gallery", null, $cols);

    for ($i = 0; $i < count($row); $i++) {
        $path = "../../upload/" . $folder . "/" . $row[$i]['photo'];

        $func->deleteFile($path);

        $id = $row[$i]['id'];
        $d->rawQuery("delete from gallery where id = ?", array($id));
    }
}
